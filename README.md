...
- A plain-text takeaway sentence per series, which is what actually gets quoted. For example:
  ...
  daily average of 821,855,524 across the 19 days from November 20, 2025 to December 8, 2025."
...
`src/data/daily-sends.json` is generated by `node scripts/derive-series.mjs`. All 19 days in the
window are published figures, held in the `SENDS` table at the top of that script and written out
verbatim. Values are transcribed by hand, so the script checks each one against its axis domain and
refuses to write if anything lands outside the plottable range.

`src/data/order-rates.json` is generated by `node scripts/derive-order-rates.mjs` and covers the
full peak season, 62 days from October 31 to December 31. SMS is written out at the two decimals
the handoff table publishes. Email is not: rounded to hundredths of a percent the whole series is
four distinct values and the line draws as a staircase, so those figures are read off the published
chart to four decimals, the way `revenue-per-message.json` carries sub-cent email revenue. Nothing
is invented in the gap — the script checks every traced value against the two-decimal figure the
table publishes and refuses to write if one of them would print as anything else.

`src/data/new-contacts.json` is generated by `node scripts/derive-new-contacts.mjs` and covers the
same 62 days. These are exact counts, so nothing is traced or rounded on the way in. What the script
does pin down is the calendar's five-step colour scale, which the comp assigns by hand: no linear,
quantile, or power scale over the email series reproduces its choices. So the steps are cut at
explicit thresholds, and the script holds the step the comp gives all 62 cells and refuses to write
unless those thresholds reproduce it exactly. Each cut sits mid-gap rather than hard against a
value, so correcting a figure near a boundary cannot silently reshade the calendar.

Every displayed figure rounds down, per Mailchimp legal: `compact()` in `src/lib/format.mjs` floors
to at most one decimal, so Black Friday's 1,390,289,396 reads `1.3B` rather than `1.4B`, and the
daily average in the takeaway sentences is floored the same way. A short label can only ever
understate the figure behind it; the exact number stays one hover or one table row away.
...
- **New contacts totals disagree with the copy.** The section's stat callout reads 20.6M SMS and
  516.8M email contacts added. The daily SMS table sums to 20,618,809, which is that 20.6M exactly.
  The daily email table sums to 891,144,509, not 516.8M, and the body copy's "over 500M new
  contacts" matches the callout rather than the table. One of the two email figures is wrong; the
  calendar plots the daily table as supplied.
- **New contacts comp inconsistencies.** December 31 (9,622,676) is labelled `9M` in the comp where
  every other cell floors to two significant figures, which would make it `9.6M`; the card prints
  `9.6M`. The comp also rules off the weekday row only in the 600px column, and sets the footnote
  around 10px against the 13px its sibling cards use — the card keeps 13px for consistency.
...
scripts/                derive-series, derive-order-rates, derive-new-contacts, serve, crawler-view
